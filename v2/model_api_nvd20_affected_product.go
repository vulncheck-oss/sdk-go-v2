/*
VulnCheck API

VulnCheck API (v3 + v4)

API version: latest
Contact: support@vulncheck.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v2

import (
	"encoding/json"
)

// checks if the ApiNVD20AffectedProduct type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ApiNVD20AffectedProduct{}

// ApiNVD20AffectedProduct api.NVD20AffectedProduct
type ApiNVD20AffectedProduct struct {
	CollectionURL *string `json:"collectionURL,omitempty"`
	Cpes []string `json:"cpes,omitempty"`
	// DefaultStatus is the status for versions not otherwise listed: one of \"affected\", \"unaffected\", or \"unknown\".
	DefaultStatus *string `json:"defaultStatus,omitempty"`
	Modules []string `json:"modules,omitempty"`
	PackageName *string `json:"packageName,omitempty"`
	PackageURL *string `json:"packageURL,omitempty"`
	Platforms []string `json:"platforms,omitempty"`
	Product *string `json:"product,omitempty"`
	ProgramFiles []string `json:"programFiles,omitempty"`
	ProgramRoutines []ApiNVD20AffectedProgramRoutine `json:"programRoutines,omitempty"`
	Repo *string `json:"repo,omitempty"`
	Vendor *string `json:"vendor,omitempty"`
	Versions []ApiNVD20AffectedVersion `json:"versions,omitempty"`
}

// NewApiNVD20AffectedProduct instantiates a new ApiNVD20AffectedProduct object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewApiNVD20AffectedProduct() *ApiNVD20AffectedProduct {
	this := ApiNVD20AffectedProduct{}
	return &this
}

// NewApiNVD20AffectedProductWithDefaults instantiates a new ApiNVD20AffectedProduct object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewApiNVD20AffectedProductWithDefaults() *ApiNVD20AffectedProduct {
	this := ApiNVD20AffectedProduct{}
	return &this
}

// GetCollectionURL returns the CollectionURL field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetCollectionURL() string {
	if o == nil || IsNil(o.CollectionURL) {
		var ret string
		return ret
	}
	return *o.CollectionURL
}

// GetCollectionURLOk returns a tuple with the CollectionURL field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetCollectionURLOk() (*string, bool) {
	if o == nil || IsNil(o.CollectionURL) {
		return nil, false
	}
	return o.CollectionURL, true
}

// HasCollectionURL returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasCollectionURL() bool {
	if o != nil && !IsNil(o.CollectionURL) {
		return true
	}

	return false
}

// SetCollectionURL gets a reference to the given string and assigns it to the CollectionURL field.
func (o *ApiNVD20AffectedProduct) SetCollectionURL(v string) {
	o.CollectionURL = &v
}

// GetCpes returns the Cpes field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetCpes() []string {
	if o == nil || IsNil(o.Cpes) {
		var ret []string
		return ret
	}
	return o.Cpes
}

// GetCpesOk returns a tuple with the Cpes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetCpesOk() ([]string, bool) {
	if o == nil || IsNil(o.Cpes) {
		return nil, false
	}
	return o.Cpes, true
}

// HasCpes returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasCpes() bool {
	if o != nil && !IsNil(o.Cpes) {
		return true
	}

	return false
}

// SetCpes gets a reference to the given []string and assigns it to the Cpes field.
func (o *ApiNVD20AffectedProduct) SetCpes(v []string) {
	o.Cpes = v
}

// GetDefaultStatus returns the DefaultStatus field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetDefaultStatus() string {
	if o == nil || IsNil(o.DefaultStatus) {
		var ret string
		return ret
	}
	return *o.DefaultStatus
}

// GetDefaultStatusOk returns a tuple with the DefaultStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetDefaultStatusOk() (*string, bool) {
	if o == nil || IsNil(o.DefaultStatus) {
		return nil, false
	}
	return o.DefaultStatus, true
}

// HasDefaultStatus returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasDefaultStatus() bool {
	if o != nil && !IsNil(o.DefaultStatus) {
		return true
	}

	return false
}

// SetDefaultStatus gets a reference to the given string and assigns it to the DefaultStatus field.
func (o *ApiNVD20AffectedProduct) SetDefaultStatus(v string) {
	o.DefaultStatus = &v
}

// GetModules returns the Modules field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetModules() []string {
	if o == nil || IsNil(o.Modules) {
		var ret []string
		return ret
	}
	return o.Modules
}

// GetModulesOk returns a tuple with the Modules field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetModulesOk() ([]string, bool) {
	if o == nil || IsNil(o.Modules) {
		return nil, false
	}
	return o.Modules, true
}

// HasModules returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasModules() bool {
	if o != nil && !IsNil(o.Modules) {
		return true
	}

	return false
}

// SetModules gets a reference to the given []string and assigns it to the Modules field.
func (o *ApiNVD20AffectedProduct) SetModules(v []string) {
	o.Modules = v
}

// GetPackageName returns the PackageName field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetPackageName() string {
	if o == nil || IsNil(o.PackageName) {
		var ret string
		return ret
	}
	return *o.PackageName
}

// GetPackageNameOk returns a tuple with the PackageName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetPackageNameOk() (*string, bool) {
	if o == nil || IsNil(o.PackageName) {
		return nil, false
	}
	return o.PackageName, true
}

// HasPackageName returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasPackageName() bool {
	if o != nil && !IsNil(o.PackageName) {
		return true
	}

	return false
}

// SetPackageName gets a reference to the given string and assigns it to the PackageName field.
func (o *ApiNVD20AffectedProduct) SetPackageName(v string) {
	o.PackageName = &v
}

// GetPackageURL returns the PackageURL field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetPackageURL() string {
	if o == nil || IsNil(o.PackageURL) {
		var ret string
		return ret
	}
	return *o.PackageURL
}

// GetPackageURLOk returns a tuple with the PackageURL field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetPackageURLOk() (*string, bool) {
	if o == nil || IsNil(o.PackageURL) {
		return nil, false
	}
	return o.PackageURL, true
}

// HasPackageURL returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasPackageURL() bool {
	if o != nil && !IsNil(o.PackageURL) {
		return true
	}

	return false
}

// SetPackageURL gets a reference to the given string and assigns it to the PackageURL field.
func (o *ApiNVD20AffectedProduct) SetPackageURL(v string) {
	o.PackageURL = &v
}

// GetPlatforms returns the Platforms field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetPlatforms() []string {
	if o == nil || IsNil(o.Platforms) {
		var ret []string
		return ret
	}
	return o.Platforms
}

// GetPlatformsOk returns a tuple with the Platforms field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetPlatformsOk() ([]string, bool) {
	if o == nil || IsNil(o.Platforms) {
		return nil, false
	}
	return o.Platforms, true
}

// HasPlatforms returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasPlatforms() bool {
	if o != nil && !IsNil(o.Platforms) {
		return true
	}

	return false
}

// SetPlatforms gets a reference to the given []string and assigns it to the Platforms field.
func (o *ApiNVD20AffectedProduct) SetPlatforms(v []string) {
	o.Platforms = v
}

// GetProduct returns the Product field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetProduct() string {
	if o == nil || IsNil(o.Product) {
		var ret string
		return ret
	}
	return *o.Product
}

// GetProductOk returns a tuple with the Product field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetProductOk() (*string, bool) {
	if o == nil || IsNil(o.Product) {
		return nil, false
	}
	return o.Product, true
}

// HasProduct returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasProduct() bool {
	if o != nil && !IsNil(o.Product) {
		return true
	}

	return false
}

// SetProduct gets a reference to the given string and assigns it to the Product field.
func (o *ApiNVD20AffectedProduct) SetProduct(v string) {
	o.Product = &v
}

// GetProgramFiles returns the ProgramFiles field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetProgramFiles() []string {
	if o == nil || IsNil(o.ProgramFiles) {
		var ret []string
		return ret
	}
	return o.ProgramFiles
}

// GetProgramFilesOk returns a tuple with the ProgramFiles field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetProgramFilesOk() ([]string, bool) {
	if o == nil || IsNil(o.ProgramFiles) {
		return nil, false
	}
	return o.ProgramFiles, true
}

// HasProgramFiles returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasProgramFiles() bool {
	if o != nil && !IsNil(o.ProgramFiles) {
		return true
	}

	return false
}

// SetProgramFiles gets a reference to the given []string and assigns it to the ProgramFiles field.
func (o *ApiNVD20AffectedProduct) SetProgramFiles(v []string) {
	o.ProgramFiles = v
}

// GetProgramRoutines returns the ProgramRoutines field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetProgramRoutines() []ApiNVD20AffectedProgramRoutine {
	if o == nil || IsNil(o.ProgramRoutines) {
		var ret []ApiNVD20AffectedProgramRoutine
		return ret
	}
	return o.ProgramRoutines
}

// GetProgramRoutinesOk returns a tuple with the ProgramRoutines field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetProgramRoutinesOk() ([]ApiNVD20AffectedProgramRoutine, bool) {
	if o == nil || IsNil(o.ProgramRoutines) {
		return nil, false
	}
	return o.ProgramRoutines, true
}

// HasProgramRoutines returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasProgramRoutines() bool {
	if o != nil && !IsNil(o.ProgramRoutines) {
		return true
	}

	return false
}

// SetProgramRoutines gets a reference to the given []ApiNVD20AffectedProgramRoutine and assigns it to the ProgramRoutines field.
func (o *ApiNVD20AffectedProduct) SetProgramRoutines(v []ApiNVD20AffectedProgramRoutine) {
	o.ProgramRoutines = v
}

// GetRepo returns the Repo field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetRepo() string {
	if o == nil || IsNil(o.Repo) {
		var ret string
		return ret
	}
	return *o.Repo
}

// GetRepoOk returns a tuple with the Repo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetRepoOk() (*string, bool) {
	if o == nil || IsNil(o.Repo) {
		return nil, false
	}
	return o.Repo, true
}

// HasRepo returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasRepo() bool {
	if o != nil && !IsNil(o.Repo) {
		return true
	}

	return false
}

// SetRepo gets a reference to the given string and assigns it to the Repo field.
func (o *ApiNVD20AffectedProduct) SetRepo(v string) {
	o.Repo = &v
}

// GetVendor returns the Vendor field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetVendor() string {
	if o == nil || IsNil(o.Vendor) {
		var ret string
		return ret
	}
	return *o.Vendor
}

// GetVendorOk returns a tuple with the Vendor field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetVendorOk() (*string, bool) {
	if o == nil || IsNil(o.Vendor) {
		return nil, false
	}
	return o.Vendor, true
}

// HasVendor returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasVendor() bool {
	if o != nil && !IsNil(o.Vendor) {
		return true
	}

	return false
}

// SetVendor gets a reference to the given string and assigns it to the Vendor field.
func (o *ApiNVD20AffectedProduct) SetVendor(v string) {
	o.Vendor = &v
}

// GetVersions returns the Versions field value if set, zero value otherwise.
func (o *ApiNVD20AffectedProduct) GetVersions() []ApiNVD20AffectedVersion {
	if o == nil || IsNil(o.Versions) {
		var ret []ApiNVD20AffectedVersion
		return ret
	}
	return o.Versions
}

// GetVersionsOk returns a tuple with the Versions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApiNVD20AffectedProduct) GetVersionsOk() ([]ApiNVD20AffectedVersion, bool) {
	if o == nil || IsNil(o.Versions) {
		return nil, false
	}
	return o.Versions, true
}

// HasVersions returns a boolean if a field has been set.
func (o *ApiNVD20AffectedProduct) HasVersions() bool {
	if o != nil && !IsNil(o.Versions) {
		return true
	}

	return false
}

// SetVersions gets a reference to the given []ApiNVD20AffectedVersion and assigns it to the Versions field.
func (o *ApiNVD20AffectedProduct) SetVersions(v []ApiNVD20AffectedVersion) {
	o.Versions = v
}

func (o ApiNVD20AffectedProduct) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ApiNVD20AffectedProduct) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CollectionURL) {
		toSerialize["collectionURL"] = o.CollectionURL
	}
	if !IsNil(o.Cpes) {
		toSerialize["cpes"] = o.Cpes
	}
	if !IsNil(o.DefaultStatus) {
		toSerialize["defaultStatus"] = o.DefaultStatus
	}
	if !IsNil(o.Modules) {
		toSerialize["modules"] = o.Modules
	}
	if !IsNil(o.PackageName) {
		toSerialize["packageName"] = o.PackageName
	}
	if !IsNil(o.PackageURL) {
		toSerialize["packageURL"] = o.PackageURL
	}
	if !IsNil(o.Platforms) {
		toSerialize["platforms"] = o.Platforms
	}
	if !IsNil(o.Product) {
		toSerialize["product"] = o.Product
	}
	if !IsNil(o.ProgramFiles) {
		toSerialize["programFiles"] = o.ProgramFiles
	}
	if !IsNil(o.ProgramRoutines) {
		toSerialize["programRoutines"] = o.ProgramRoutines
	}
	if !IsNil(o.Repo) {
		toSerialize["repo"] = o.Repo
	}
	if !IsNil(o.Vendor) {
		toSerialize["vendor"] = o.Vendor
	}
	if !IsNil(o.Versions) {
		toSerialize["versions"] = o.Versions
	}
	return toSerialize, nil
}

type NullableApiNVD20AffectedProduct struct {
	value *ApiNVD20AffectedProduct
	isSet bool
}

func (v NullableApiNVD20AffectedProduct) Get() *ApiNVD20AffectedProduct {
	return v.value
}

func (v *NullableApiNVD20AffectedProduct) Set(val *ApiNVD20AffectedProduct) {
	v.value = val
	v.isSet = true
}

func (v NullableApiNVD20AffectedProduct) IsSet() bool {
	return v.isSet
}

func (v *NullableApiNVD20AffectedProduct) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableApiNVD20AffectedProduct(val *ApiNVD20AffectedProduct) *NullableApiNVD20AffectedProduct {
	return &NullableApiNVD20AffectedProduct{value: val, isSet: true}
}

func (v NullableApiNVD20AffectedProduct) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableApiNVD20AffectedProduct) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


