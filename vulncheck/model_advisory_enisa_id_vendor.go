/*
VulnCheck API

Version 3 of the VulnCheck API

API version: 3.0
Contact: support@vulncheck.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package vulncheck

import (
	"encoding/json"
)

// checks if the AdvisoryEnisaIDVendor type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AdvisoryEnisaIDVendor{}

// AdvisoryEnisaIDVendor struct for AdvisoryEnisaIDVendor
type AdvisoryEnisaIDVendor struct {
	Id *string `json:"id,omitempty"`
	VendorName *string `json:"vendor_name,omitempty"`
}

// NewAdvisoryEnisaIDVendor instantiates a new AdvisoryEnisaIDVendor object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAdvisoryEnisaIDVendor() *AdvisoryEnisaIDVendor {
	this := AdvisoryEnisaIDVendor{}
	return &this
}

// NewAdvisoryEnisaIDVendorWithDefaults instantiates a new AdvisoryEnisaIDVendor object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAdvisoryEnisaIDVendorWithDefaults() *AdvisoryEnisaIDVendor {
	this := AdvisoryEnisaIDVendor{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *AdvisoryEnisaIDVendor) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryEnisaIDVendor) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *AdvisoryEnisaIDVendor) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *AdvisoryEnisaIDVendor) SetId(v string) {
	o.Id = &v
}

// GetVendorName returns the VendorName field value if set, zero value otherwise.
func (o *AdvisoryEnisaIDVendor) GetVendorName() string {
	if o == nil || IsNil(o.VendorName) {
		var ret string
		return ret
	}
	return *o.VendorName
}

// GetVendorNameOk returns a tuple with the VendorName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryEnisaIDVendor) GetVendorNameOk() (*string, bool) {
	if o == nil || IsNil(o.VendorName) {
		return nil, false
	}
	return o.VendorName, true
}

// HasVendorName returns a boolean if a field has been set.
func (o *AdvisoryEnisaIDVendor) HasVendorName() bool {
	if o != nil && !IsNil(o.VendorName) {
		return true
	}

	return false
}

// SetVendorName gets a reference to the given string and assigns it to the VendorName field.
func (o *AdvisoryEnisaIDVendor) SetVendorName(v string) {
	o.VendorName = &v
}

func (o AdvisoryEnisaIDVendor) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AdvisoryEnisaIDVendor) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.VendorName) {
		toSerialize["vendor_name"] = o.VendorName
	}
	return toSerialize, nil
}

type NullableAdvisoryEnisaIDVendor struct {
	value *AdvisoryEnisaIDVendor
	isSet bool
}

func (v NullableAdvisoryEnisaIDVendor) Get() *AdvisoryEnisaIDVendor {
	return v.value
}

func (v *NullableAdvisoryEnisaIDVendor) Set(val *AdvisoryEnisaIDVendor) {
	v.value = val
	v.isSet = true
}

func (v NullableAdvisoryEnisaIDVendor) IsSet() bool {
	return v.isSet
}

func (v *NullableAdvisoryEnisaIDVendor) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAdvisoryEnisaIDVendor(val *AdvisoryEnisaIDVendor) *NullableAdvisoryEnisaIDVendor {
	return &NullableAdvisoryEnisaIDVendor{value: val, isSet: true}
}

func (v NullableAdvisoryEnisaIDVendor) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAdvisoryEnisaIDVendor) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


