/*
VulnCheck API

Version 3 of the VulnCheck API

API version: 3.0
Contact: support@vulncheck.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package vulncheck

import (
	"encoding/json"
)

// checks if the AdvisoryDebianCVE type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AdvisoryDebianCVE{}

// AdvisoryDebianCVE struct for AdvisoryDebianCVE
type AdvisoryDebianCVE struct {
	Cve *string `json:"cve,omitempty"`
	Debianbug *int32 `json:"debianbug,omitempty"`
	Description *string `json:"description,omitempty"`
	Releases []AdvisoryAffectedDebianRelease `json:"releases,omitempty"`
	Scope *string `json:"scope,omitempty"`
	Url *string `json:"url,omitempty"`
}

// NewAdvisoryDebianCVE instantiates a new AdvisoryDebianCVE object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAdvisoryDebianCVE() *AdvisoryDebianCVE {
	this := AdvisoryDebianCVE{}
	return &this
}

// NewAdvisoryDebianCVEWithDefaults instantiates a new AdvisoryDebianCVE object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAdvisoryDebianCVEWithDefaults() *AdvisoryDebianCVE {
	this := AdvisoryDebianCVE{}
	return &this
}

// GetCve returns the Cve field value if set, zero value otherwise.
func (o *AdvisoryDebianCVE) GetCve() string {
	if o == nil || IsNil(o.Cve) {
		var ret string
		return ret
	}
	return *o.Cve
}

// GetCveOk returns a tuple with the Cve field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryDebianCVE) GetCveOk() (*string, bool) {
	if o == nil || IsNil(o.Cve) {
		return nil, false
	}
	return o.Cve, true
}

// HasCve returns a boolean if a field has been set.
func (o *AdvisoryDebianCVE) HasCve() bool {
	if o != nil && !IsNil(o.Cve) {
		return true
	}

	return false
}

// SetCve gets a reference to the given string and assigns it to the Cve field.
func (o *AdvisoryDebianCVE) SetCve(v string) {
	o.Cve = &v
}

// GetDebianbug returns the Debianbug field value if set, zero value otherwise.
func (o *AdvisoryDebianCVE) GetDebianbug() int32 {
	if o == nil || IsNil(o.Debianbug) {
		var ret int32
		return ret
	}
	return *o.Debianbug
}

// GetDebianbugOk returns a tuple with the Debianbug field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryDebianCVE) GetDebianbugOk() (*int32, bool) {
	if o == nil || IsNil(o.Debianbug) {
		return nil, false
	}
	return o.Debianbug, true
}

// HasDebianbug returns a boolean if a field has been set.
func (o *AdvisoryDebianCVE) HasDebianbug() bool {
	if o != nil && !IsNil(o.Debianbug) {
		return true
	}

	return false
}

// SetDebianbug gets a reference to the given int32 and assigns it to the Debianbug field.
func (o *AdvisoryDebianCVE) SetDebianbug(v int32) {
	o.Debianbug = &v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *AdvisoryDebianCVE) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryDebianCVE) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *AdvisoryDebianCVE) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *AdvisoryDebianCVE) SetDescription(v string) {
	o.Description = &v
}

// GetReleases returns the Releases field value if set, zero value otherwise.
func (o *AdvisoryDebianCVE) GetReleases() []AdvisoryAffectedDebianRelease {
	if o == nil || IsNil(o.Releases) {
		var ret []AdvisoryAffectedDebianRelease
		return ret
	}
	return o.Releases
}

// GetReleasesOk returns a tuple with the Releases field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryDebianCVE) GetReleasesOk() ([]AdvisoryAffectedDebianRelease, bool) {
	if o == nil || IsNil(o.Releases) {
		return nil, false
	}
	return o.Releases, true
}

// HasReleases returns a boolean if a field has been set.
func (o *AdvisoryDebianCVE) HasReleases() bool {
	if o != nil && !IsNil(o.Releases) {
		return true
	}

	return false
}

// SetReleases gets a reference to the given []AdvisoryAffectedDebianRelease and assigns it to the Releases field.
func (o *AdvisoryDebianCVE) SetReleases(v []AdvisoryAffectedDebianRelease) {
	o.Releases = v
}

// GetScope returns the Scope field value if set, zero value otherwise.
func (o *AdvisoryDebianCVE) GetScope() string {
	if o == nil || IsNil(o.Scope) {
		var ret string
		return ret
	}
	return *o.Scope
}

// GetScopeOk returns a tuple with the Scope field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryDebianCVE) GetScopeOk() (*string, bool) {
	if o == nil || IsNil(o.Scope) {
		return nil, false
	}
	return o.Scope, true
}

// HasScope returns a boolean if a field has been set.
func (o *AdvisoryDebianCVE) HasScope() bool {
	if o != nil && !IsNil(o.Scope) {
		return true
	}

	return false
}

// SetScope gets a reference to the given string and assigns it to the Scope field.
func (o *AdvisoryDebianCVE) SetScope(v string) {
	o.Scope = &v
}

// GetUrl returns the Url field value if set, zero value otherwise.
func (o *AdvisoryDebianCVE) GetUrl() string {
	if o == nil || IsNil(o.Url) {
		var ret string
		return ret
	}
	return *o.Url
}

// GetUrlOk returns a tuple with the Url field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryDebianCVE) GetUrlOk() (*string, bool) {
	if o == nil || IsNil(o.Url) {
		return nil, false
	}
	return o.Url, true
}

// HasUrl returns a boolean if a field has been set.
func (o *AdvisoryDebianCVE) HasUrl() bool {
	if o != nil && !IsNil(o.Url) {
		return true
	}

	return false
}

// SetUrl gets a reference to the given string and assigns it to the Url field.
func (o *AdvisoryDebianCVE) SetUrl(v string) {
	o.Url = &v
}

func (o AdvisoryDebianCVE) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AdvisoryDebianCVE) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Cve) {
		toSerialize["cve"] = o.Cve
	}
	if !IsNil(o.Debianbug) {
		toSerialize["debianbug"] = o.Debianbug
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.Releases) {
		toSerialize["releases"] = o.Releases
	}
	if !IsNil(o.Scope) {
		toSerialize["scope"] = o.Scope
	}
	if !IsNil(o.Url) {
		toSerialize["url"] = o.Url
	}
	return toSerialize, nil
}

type NullableAdvisoryDebianCVE struct {
	value *AdvisoryDebianCVE
	isSet bool
}

func (v NullableAdvisoryDebianCVE) Get() *AdvisoryDebianCVE {
	return v.value
}

func (v *NullableAdvisoryDebianCVE) Set(val *AdvisoryDebianCVE) {
	v.value = val
	v.isSet = true
}

func (v NullableAdvisoryDebianCVE) IsSet() bool {
	return v.isSet
}

func (v *NullableAdvisoryDebianCVE) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAdvisoryDebianCVE(val *AdvisoryDebianCVE) *NullableAdvisoryDebianCVE {
	return &NullableAdvisoryDebianCVE{value: val, isSet: true}
}

func (v NullableAdvisoryDebianCVE) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAdvisoryDebianCVE) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


