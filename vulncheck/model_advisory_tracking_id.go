/*
VulnCheck API

Version 3 of the VulnCheck API

API version: 3.0
Contact: support@vulncheck.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package vulncheck

import (
	"encoding/json"
)

// checks if the AdvisoryTrackingID type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AdvisoryTrackingID{}

// AdvisoryTrackingID struct for AdvisoryTrackingID
type AdvisoryTrackingID struct {
	SystemName *string `json:"system_name,omitempty"`
	Text *string `json:"text,omitempty"`
}

// NewAdvisoryTrackingID instantiates a new AdvisoryTrackingID object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAdvisoryTrackingID() *AdvisoryTrackingID {
	this := AdvisoryTrackingID{}
	return &this
}

// NewAdvisoryTrackingIDWithDefaults instantiates a new AdvisoryTrackingID object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAdvisoryTrackingIDWithDefaults() *AdvisoryTrackingID {
	this := AdvisoryTrackingID{}
	return &this
}

// GetSystemName returns the SystemName field value if set, zero value otherwise.
func (o *AdvisoryTrackingID) GetSystemName() string {
	if o == nil || IsNil(o.SystemName) {
		var ret string
		return ret
	}
	return *o.SystemName
}

// GetSystemNameOk returns a tuple with the SystemName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryTrackingID) GetSystemNameOk() (*string, bool) {
	if o == nil || IsNil(o.SystemName) {
		return nil, false
	}
	return o.SystemName, true
}

// HasSystemName returns a boolean if a field has been set.
func (o *AdvisoryTrackingID) HasSystemName() bool {
	if o != nil && !IsNil(o.SystemName) {
		return true
	}

	return false
}

// SetSystemName gets a reference to the given string and assigns it to the SystemName field.
func (o *AdvisoryTrackingID) SetSystemName(v string) {
	o.SystemName = &v
}

// GetText returns the Text field value if set, zero value otherwise.
func (o *AdvisoryTrackingID) GetText() string {
	if o == nil || IsNil(o.Text) {
		var ret string
		return ret
	}
	return *o.Text
}

// GetTextOk returns a tuple with the Text field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvisoryTrackingID) GetTextOk() (*string, bool) {
	if o == nil || IsNil(o.Text) {
		return nil, false
	}
	return o.Text, true
}

// HasText returns a boolean if a field has been set.
func (o *AdvisoryTrackingID) HasText() bool {
	if o != nil && !IsNil(o.Text) {
		return true
	}

	return false
}

// SetText gets a reference to the given string and assigns it to the Text field.
func (o *AdvisoryTrackingID) SetText(v string) {
	o.Text = &v
}

func (o AdvisoryTrackingID) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AdvisoryTrackingID) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.SystemName) {
		toSerialize["system_name"] = o.SystemName
	}
	if !IsNil(o.Text) {
		toSerialize["text"] = o.Text
	}
	return toSerialize, nil
}

type NullableAdvisoryTrackingID struct {
	value *AdvisoryTrackingID
	isSet bool
}

func (v NullableAdvisoryTrackingID) Get() *AdvisoryTrackingID {
	return v.value
}

func (v *NullableAdvisoryTrackingID) Set(val *AdvisoryTrackingID) {
	v.value = val
	v.isSet = true
}

func (v NullableAdvisoryTrackingID) IsSet() bool {
	return v.isSet
}

func (v *NullableAdvisoryTrackingID) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAdvisoryTrackingID(val *AdvisoryTrackingID) *NullableAdvisoryTrackingID {
	return &NullableAdvisoryTrackingID{value: val, isSet: true}
}

func (v NullableAdvisoryTrackingID) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAdvisoryTrackingID) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


